from tarea14.config.mysqlconnection import conectarMySQL
...
class Usuarios:
    ...
    def obtener_todo(cls):
        ...
        results = conectarMySQL('recetas_schema').query_db(query) #result serian un diccionario en donde conectamos con el nombre de nuestra base de datos y  vamos a llamar a la función conectarMySQL con el esquema al que te diriges
...
#METODO CREATE con INSERT
    ...
    def registro_usuario(cls, data): #(nombre de las columnas en nuestra tabla y en VALUES nombre de las claves de nuestro diccionario del controlador de forma sanitizada)
        ...
        return conectarMySQL('recetas_schema').query_db(query, data)
...
#para obtener un usuario a traves de su id
    ...
    def obtener_un_usuario(cls, data):
        ...
        results =  conectarMySQL('recetas_schema').query_db(query, data)
        if len(results) < 1:
            return False
...
#a traves del email estamos obteniendo un usuario
    ...
    def obtener_por_email(cls,data):
        ...
        result = conectarMySQL('recetas_schema').query_db(query, data)
    ...
    def validar_formulario(usuario):
        ...
        if usuario['contraseña'] != usuario['confirmar']:
            ...
            validar = False
        if not EMAIL_REGEX.match(datos_email['email']): #aca validamos con el formato con expresion regular de una direcion del mail #antes lo validamos match(usuario['email'])
            flash("ATENCIÓN ¡Direccion de EMAIL no es valido!") #enviamos el valor de la clave datos_email 
            validar = False
        elif Usuarios.obtener_por_email(datos_email): #elif esta conectado a la pregunta de email, consultamos si existe en la bd
